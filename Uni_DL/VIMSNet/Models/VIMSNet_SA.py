...
class VIMSNet(nn.Module):
    def __init__(self, args):
        ...
        if args.data_type == "SA_Drowsy_bal" or args.data_type == "SA_Drowsy_unbal":

            self.excitation = nn.Sequential(
                nn.Linear(192, 5), # original = nn.Linear(10,5)
                nn.ReLU(inplace=True),
                nn.Linear(5, 192), # original = nn.Linear(5,10)
                nn.Sigmoid(),
            )
            self.dense_1 = nn.Sequential(
                nn.Linear(9216, 512), # original: nn.Linear(48*10, 512),
                nn.ReLU(inplace=True),
                nn.Dropout(0.3)
            )

            self.dense_2 = nn.Sequential(
                nn.Linear(12032, 128),  # original: nn.Linear(532, 128),
                nn.ReLU(inplace=True),
                nn.Dropout(0.3)
        )
        else:    
            self.excitation = nn.Sequential(
                nn.Linear(375, 5), # original = nn.Linear(10,5)
                nn.ReLU(inplace=True),
                nn.Linear(5, 375), # original = nn.Linear(5,10)
                nn.Sigmoid(),
            )
            self.dense_1 = nn.Sequential(
                nn.Linear(18000, 512), # original: nn.Linear(48*10, 512),
                nn.ReLU(inplace=True),
                nn.Dropout(0.3)
            )

            self.dense_2 = nn.Sequential(
                nn.Linear(21512, 128),  # original: nn.Linear(532, 128),
                nn.ReLU(inplace=True),
                nn.Dropout(0.3)
            )
