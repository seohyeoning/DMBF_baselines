...
class tfn(nn.Module):
    def __init__(self, args):
        ...
        self.dense2 = nn.Linear(32, self.args.n_classes)
...
class Net(nn.Module):
    ...
    def forward(self, inputs):
        ...
        (xe_fe, xc_fe, xg_fe, xr_fe) =  self.sub_fe1(xe), \
                                        self.sub_fe2(xc.unsqueeze(dim=1)), \
                                        self.sub_fe3(xr.unsqueeze(dim=1)), \
                                        self.sub_fe4(xg.unsqueeze(dim=1))
        
        (x_attn_e, x_attn_cg, x_attn_r) = self.sub_attn1(xe_fe), \
                                           self.sub_attn2([xc_fe, xg_fe]), \
                                           self.sub_attn3(xr_fe)
        output = self.tfn([x_attn_e, x_attn_cg, x_attn_r])
